...
begin
  ...
  if (delta1=0 )and (delta2=0) then
    ...
    begin
       ...
       if Y2>0 then expY2 := exp((1/3)*ln(Y2))
          else expY2 := (-1)*exp((1/3)*ln(abs(Y2)));
           {
                   ...
                   writeln('x2=',(-2*b+exp((1/3)*ln(Y1)) +exp((1/3)*ln(Y2)))/(6*a):0:5,'+',sqrt(3)* (exp((1/3)*ln(Y1))-exp((1/3)*ln(Y2)))/(6*a):0:5,'i');
                   writeln('x3=',(-2*b+exp((1/3)*ln(Y1)) +exp((1/3)*ln(Y2)))/(6*a):0:5,'-',sqrt(3)* (exp((1/3)*ln(Y1))-exp((1/3)*ln(Y2)))/(6*a):0:5,'i')
           }
       writeln('x1=',(-b-expY1 - expY2)/(3*a):0:5);
       writeln('x2=',(-2*b+expY1 +expY2)/(6*a):0:5,'+',sqrt(3)* (expY1-expY2)/(6*a):0:5,'i');
       writeln('x3=',(-2*b+expY1 +expY2)/(6*a):0:5,'-',sqrt(3)* (expY1-expY2)/(6*a):0:5,'i')
    end
  else if delta=0 then
    begin
    ...
    begin
       ...
       writeln('X2=',(-b+sqrt(delta1)*(cos(theta/3)+sqrt(3)*sin(theta/3)))/(3*a):0:5);
       writeln('X3=',(-b+sqrt(delta1)*(cos(theta/3)-sqrt(3)*sin(theta/3)))/(3*a):0:5)
    end;
end.
